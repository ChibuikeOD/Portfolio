...
def app():
    ...
    **Controls Guide:**
    ...
    tab1, tab2 = st.tabs(["Contour Visualization", "Project Report"])
    ...
    with tab2:
        st.markdown("""
        ### Project Report: CT Scan Contour Analysis
        
        #### 1. Data Source
        - **Data**: Brain CT (Computed Tomography) scans stored in DICOM format and converted to JSON.
        - **Structure**: The data grid represents tissue density values (Hounsfield units).
        
        #### 2. Visualization Technique
        - **Algorithm**: **Marching Squares**. 
            - This is a computer graphics algorithm that generates contour lines for a two-dimensional scalar field (density grid).
            - It calculates where the "isoline" (line of constant density) passes through each grid cell.
        - **Implementation**: Custom **D3.js** implementation.
        
        #### 3. Interactive Features
        - **Brain Scan Selector**: Users can toggle between different brain slices (e.g., `brain_001.dcm`, `brain_002.dcm`) using the dropdown.
        - **Threshold Controls**:
            - **Min/Max Threshold**: Filters the display to show only tissues within a specific density range (e.g., isolating bone vs. soft tissue).
            - **Bin Count**: Controls the granularity of the contours. Higher bin counts produce smoother, more detailed maps.
        """)
