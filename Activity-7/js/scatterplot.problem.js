                                 ...
                                 margin = 50)
{
    ...
    legends_items
        .on("click", (event, d) => {
            const isSelected = d3.select(event.currentTarget).select("rect").classed("legendSelect");
            legends_items.selectAll("rect").classed("legendSelect", false);
            if (!isSelected) {
                d3.select(event.currentTarget).select("rect").classed("legendSelect", true);
                d3.selectAll("circle").classed("selected", false);
                d3.selectAll(`circle.${d}`).classed("selected", true);
            } else {
                d3.selectAll("circle").classed("selected", false);
            }
        });
